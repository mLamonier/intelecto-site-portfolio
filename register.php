...
$tipoRegistro = $_GET['tipo'] ?? 'aluno'; 
if ($tipoRegistro === 'admin' && !isAdminSession()) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!CSRF::validate()) {
        ...
        if ($nome === '' || $email === '' || $cpf === '' || $telefone === '' || $senha === '' || $confirm === '') {
            ...
            $erro = 'A senha não atende aos critérios de segurança mínimos.';
        } else {
            try {
                $db->beginTransaction();
                ...
                if ($stmt->fetch()) {
                    ...
                    $stmt = $db->prepare('
                        ...
                        VALUES (:nome, :email, :senha_hash, :telefone, 1, NOW())
                    ');
                    ...
                    $role = ($tipoRegistro === 'admin') ? 'ADMIN' : 'ALUNO';
                    ...
                    $stmtRole->bindValue(':role', $role);
                    $stmtRole->execute();

                    $db->commit();
                    header('Location: ' . site_path('login.php?registered=1'));
                    exit;
                }
...
$tipoRegistro = ($tipoRegistro === 'admin' && isAdminSession()) ? 'admin' : 'aluno';
$tituloRegistro = ($tipoRegistro === 'admin') ? 'Criar Conta Admin' : 'Criar Conta';
$descricaoRegistro = ($tipoRegistro === 'admin') ? 'Cadastre um novo administrador.' : 'Crie sua conta e comece agora.';
...
<head>
    ...
    <title><?= htmlspecialchars($tituloRegistro) ?> - Intelecto</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container">
        <h1><?= htmlspecialchars($tituloRegistro) ?></h1>
        <p><?= htmlspecialchars($descricaoRegistro) ?></p>
        <?php if ($erro): ?>
        ...
        <form method="POST" action="">
            ...
            <div class="privacy-note">
                ...
                <a href="<?= htmlspecialchars($frontendBaseUrl) ?>/politica-de-privacidade">política de privacidade</a> e
                <a href="<?= htmlspecialchars($frontendBaseUrl) ?>/termos-de-uso">termos de uso</a>.
            </div>
    ...
    <script>
        ...
        const btnSubmit = document.getElementById('btnCriarConta');
        ...
        function calcularForta(validacoes) {
        ...
        inputSenha.addEventListener('input', function() {
            ...
            const forca = calcularForta(validacoes);
            ...
            const textosForta = {
                ...
                strong: '🟢 Senha forte - muito segura'
            };
        ...
        document.querySelector('form').addEventListener('submit', function(e) {
            ...
            const validacoes = validarSenha(senha);
            const todasAtendidas = Object.values(validacoes).every(v => v);
